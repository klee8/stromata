...
for i in `cat genome_tag_list.txt`
  do
    mkdir results/$i
    cp ../exonerate/align/$i/$i.masked.fa results/$i
    ...
    echo "docker run -u $(id -u) -v $(pwd):/results/$i -w /results/$i/ ezlabgva/busco:v4.1.1_cv1 busco -i results/$i/$i.masked.fa --config=myconfig.ini --out $i --mode genome --lineage eukaryota_odb10 -f" >> run_busco_docker.sh 
