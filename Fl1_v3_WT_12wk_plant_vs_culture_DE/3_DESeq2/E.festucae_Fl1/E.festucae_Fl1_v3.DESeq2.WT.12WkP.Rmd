---
title: "E.festucae_Fl1_v3_WT_vs_12WkP"
...
samples <- samples[samples$pop == "E.festucae_Fl1",]

# subset conditions of interest
samples <- samples[samples$condition == "WT_12WkP" | samples$condition == "WT_CULT",]
samples
```
...
tx2gene <- read.table(file.path(dir, "transcriptome/E.festucae_Fl1/transcriptome.headers.txt"))
...
salmon_TPM <- as.data.frame (txi$abundance) %>% 
  mutate( mean_WT_12WkP_TPM = ((SRR7640294 + SRR7640295 + SRR7640296 + SRR7640297)/4),
          mean_WT_CULT_TPM = ((SRR7640306 + SRR7640308 + SRR7640309)/3),
          gene_id = rownames(txi$abundance)) %>%
  select(gene_id, mean_WT_12WkP_TPM, mean_WT_CULT_TPM)
...
pdf("plots/WT_12WkP_vs_WT_CULT_rlog2_normalised_log2FC.pdf", onefile = FALSE)
...
pdf("plots/WT_12WkP_vs_WT_CULT_heat_map_sample_dist_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/WT_12WkP_vs_WT_CULT_PCA_plot_rld-normalised_data.pdf", onefile = FALSE)
...
dds$condition <- relevel(dds$condition, ref = "WT_CULT")
```
...
pdf("plots/WT_12WkP_vs_WT_CULT_top_DE_genes.pdf" )
...
```{r plot_dispersions}
pdf("plots/WT_12WkP_vs_WT_CULT_plot_dispersions.pdf", onefile = FALSE)
plotDispEsts(dds)
dev.off()
```
...
resApeT_thresh1  <- lfcShrink(dds, coef="condition_WT_12WkP_vs_WT_CULT", type="apeglm", lfcThreshold = 1)
resApeT_thresh2  <- lfcShrink(dds, coef="condition_WT_12WkP_vs_WT_CULT", type="apeglm", lfcThreshold = 2)


pdf("plots/WT_12WkP_vs_WT_CULT_apeglm_shrinkage_MA_plot.pdf", onefile = FALSE)
...
pdf("plots/WT_12WkP_vs_WT_CULT_volcano_plot.pdf", onefile = FALSE)
...
# merge in mean fpkm count data from DESEQ2
...
m3 <- read.delim("../../../Fl1_v3_histone_mutants_DE/protein_sequences/Fl1_v3_to_M3_conversion.txt", header = TRUE, sep = "\t")
m3$gene_id <- gsub("-T1", "", m3$EfeFl1_proteins_v3.faa)
#m3$m3_genes <- gsub(".mRNA-([0-9])", "", m3$p3.v7.2014.03.27.faa)
m3_keep <- m3[, c("gene_id", "p3.v7.2014.03.27.faa", "EfeFl1_other_hits")]
colnames(m3_keep) <- c("gene_id", "m3_orthologs", "EfeFl1_other_hits")
all_results <- merge(all_results, m3_keep, by = "gene_id", all = TRUE)

head(all_results)
goi <- all_results %>% select(gene_id, apeglm_1_log2FC, apeglm_1_lfcSE, apeglm_1_svalue, apeglm_2_svalue, mean_WT_12WkP_TPM, mean_WT_CULT_TPM, m3_orthologs, EfeFl1_other_hits)
colnames(goi) <- c("gene_id", "12WkvCULT_apeglm_log2FC", "12WkvCULT_apeglm_lfcSE", "12WkvCULT_apeglm_1_svalue", "12WkvCULT_apeglm_2_svalue", "mean_WT_12WkP_TPM", "mean_WT_CULT_TPM", "m3_orthologs", "EfeFl1_other_hits")
head(goi)
write.table(goi, "result_tables/E.festucae_Fl1_v3_WT_12WkP_vs_WT_CULT.txt", quote = FALSE, row.names = FALSE, sep = "\t")
```
