---
title: "E.elymi_NfE728_STR_vs_PS"
...
samples <- samples[samples$condition == "PS" | samples$condition == "STR",]
```
...
```
Transcripts Per Million (TPM) is a normalization method for RNA-seq, should be read as "for every 1,000,000 RNA molecules in the RNA-seq sample, x came from this gene/transcript."  ~ ArrayServer.com
Here Salmon has calcuated TPM for each sample
...
PS_list <- as.vector(samples[samples$condition == "PS", "run"])
...
PS_samplenum <- length(as.vector(unique(samples[samples$condition == "PS", "sample"])))

salmon_TPM <- as.data.frame (txi$abundance) %>% 
  mutate( mean_STR_TPM = rowSums(select(., STR_list)/STR_samplenum),
          mean_PS_TPM = rowSums(select(., PS_list)/PS_samplenum),
          gene_id = rownames(txi$abundance)) %>%
  select(gene_id, mean_STR_TPM, mean_PS_TPM) 
```
...
Use regularized-logarithm transformation or rlog (Love, Huber, and Anders 2014) to normalise raw data prior to creating PCA and checking sample distance. An alternative transformation is vst, which is faster)
...
pdf("plots/STR_vs_PS_rlog2_normalised_log2FC.pdf", onefile = FALSE)
...
pdf("plots/STR_vs_PS_heat_map_sample_dist_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/STR_v_PS_PCA_plot_rld-normalised_data.pdf", onefile = FALSE)
...
If you never tell the DESeq2 functions which level you want to compare against (e.g. which level represents the control group), the comparisons will be based on the alphabetical order of the levels. Here just set the reference condition to stem
```{r set_ref_condition}

dds$condition <- relevel(dds$condition, ref = "PS")
```
...
pdf("plots/STR_vs_PS_top_DE_genes.pdf" )
...
```{r plot_dispersions}
pdf("plots/STR_vs_PS_plot_dispersions.pdf", onefile = FALSE)
plotDispEsts(dds)
dev.off()
```
...
resApeT_thresh1  <- lfcShrink(dds, coef="condition_STR_vs_PS", type="apeglm", lfcThreshold = 1)
resApeT_thresh2  <- lfcShrink(dds, coef="condition_STR_vs_PS", type="apeglm", lfcThreshold = 2)


pdf("plots/STR_vs_PS_apeglm_shrinkage_MA_plot.pdf", onefile = FALSE)
...
pdf("plots/STR_vs_PS_volcano_plot.pdf", onefile = FALSE)
...
goi <- all_results %>% select(gene_id, apeglm_log2FC, apeglm_lfcSE, apeglm_1_svalue, apeglm_2_svalue, mean_PS_TPM, mean_STR_TPM)



write.table(goi, "result_tables/E.elymi_NfE728_STR_PS.txt", quote = FALSE, row.names = FALSE, sep = "\t")
```
