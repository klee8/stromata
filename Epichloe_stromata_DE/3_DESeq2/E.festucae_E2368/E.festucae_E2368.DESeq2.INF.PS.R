...
samples <- samples[samples$condition == "PS" | samples$condition == "INF",]
...
salmon_TPM <- as.data.frame (txi$abundance) %>% 
  mutate( mean_INF_TPM = rowSums(select(., INF_list)/INF_samplenum),
          mean_PS_TPM = rowSums(select(., PS_list)/PS_samplenum),
          gene_id = rownames(txi$abundance)) %>%
  select(gene_id, mean_INF_TPM, mean_PS_TPM) 
...
pdf("plots/INF_vs_PS_rlog2_normalised_log2FC.pdf", onefile = FALSE)
...
pdf("plots/INF_vs_PS_heat_map_sample_dist_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/INF_v_PS_PCA_plot_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/INF_vs_PS_top_DE_genes.pdf" )
...
# plot_dispersions
pdf("plots/INF_vs_PS_plot_dispersions.pdf", onefile = FALSE)
plotDispEsts(dds)
dev.off()
...
resApeT_thresh1  <- lfcShrink(dds, coef="condition_INF_vs_PS", type="apeglm", lfcThreshold = 1)
resApeT_thresh2  <- lfcShrink(dds, coef="condition_INF_vs_PS", type="apeglm", lfcThreshold = 2)

pdf("plots/INF_vs_PS_apeglm_shrinkage_MA_plot.pdf", onefile = FALSE)
...
# volcano plot
pdf("plots/INF_vs_PS_volcano_plot.pdf", onefile = FALSE)
#reset par
par(mfrow=c(1,1))
...
goi <- all_results %>% select(gene_id, apeglm_log2FC, apeglm_lfcSE, apeglm_1_svalue, apeglm_2_svalue, mean_PS_TPM, mean_INF_TPM)

write.table(goi, "result_tables/E.festucae_E2368_INF_PS.txt", quote = FALSE, row.names = FALSE, sep = "\t")
...
write.table(sub_2FC, "result_tables/E.festucae_E2368_INF_PS_2FC.txt", quote = FALSE, row.names = FALSE, sep = "\t")
