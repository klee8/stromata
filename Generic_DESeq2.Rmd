...
samples <- samples[samples$pop == "SPECIES",]

# subset conditions of interest
samples <- samples[samples$condition == "REF" | samples$condition == "SOI",]
```
...
tx2gene <- read.table(file.path(dir, "transcriptome/SPECIES/transcriptome.headers.txt"))
...
pdf("plots/SOI_vs_REF_rlog2_normalised_log2FC.pdf", onefile = FALSE)
...
pdf("plots/SOI_vs_REF_heat_map_sample_dist_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/SOI_vs_REF_PCA_plot_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/SOI_vs_REF_top_DE_genes.pdf" )
...
```{r plot_dispersions}
pdf("plots/SOI_vs_REF_plot_dispersions.pdf", onefile = FALSE)
plotDispEsts(dds)
dev.off()
```
...
resApeT_thresh1  <- lfcShrink(dds, coef="condition_SOI_vs_REF", type="apeglm", lfcThreshold = 1)
resApeT_thresh2  <- lfcShrink(dds, coef="condition_SOI_vs_REF", type="apeglm", lfcThreshold = 2)


pdf("plots/SOI_vs_REF_apeglm_shrinkage_MA_plot.pdf", onefile = FALSE)
...
pdf("plots/SOI_vs_REF_volcano_plot.pdf", onefile = FALSE)
...
write.table(all_results, "result_tables/SPECIES_SOI_REF.txt", quote = FALSE, row.names = FALSE, sep = "\t")
```
