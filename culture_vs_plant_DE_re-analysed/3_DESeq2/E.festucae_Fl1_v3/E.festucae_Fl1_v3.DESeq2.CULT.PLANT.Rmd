---
title: "E.festucae_Fl1_v3_CULT_vs_PLANT"
...
samples <- samples[samples$pop == "E.festucae_Fl1_v3",]

# subset conditions of interest
samples <- samples[samples$condition == "PLANT" | samples$condition == "CULT",]
```
...
tx2gene <- read.table(file.path(dir, "transcriptome/E.festucae_Fl1_v3/transcriptome.headers.txt"))
...
pdf("plots/CULT_vs_PLANT_rlog2_normalised_log2FC.pdf", onefile = FALSE)
...
pdf("plots/CULT_vs_PLANT_heat_map_sample_dist_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/CULT_v_PLANT_PCA_plot_rld-normalised_data.pdf", onefile = FALSE)
...
dds$condition <- relevel(dds$condition, ref = "PLANT")
```
...
pdf("plots/CULT_vs_PLANT_top_DE_genes.pdf" )
...
```{r plot_dispersions}
pdf("plots/CULT_vs_PLANT_plot_dispersions.pdf", onefile = FALSE)
plotDispEsts(dds)
dev.off()
```
...
resApeT_thresh1  <- lfcShrink(dds, coef="condition_CULT_vs_PLANT", type="apeglm", lfcThreshold = 1)
resApeT_thresh2  <- lfcShrink(dds, coef="condition_CULT_vs_PLANT", type="apeglm", lfcThreshold = 2)


pdf("plots/CULT_vs_PLANT_apeglm_shrinkage_MA_plot.pdf", onefile = FALSE)
...
pdf("plots/CULT_vs_PLANT_volcano_plot.pdf", onefile = FALSE)
...
write.table(all_results, "result_tables/E.festucae_Fl1_v3_CULT_PLANT.txt", quote = FALSE, row.names = FALSE, sep = "\t")
```
